...
Read [Operational Safety](/guides/operational-safety) before powering the robot for the first time, including the [additional hazards specific to humanoids](/guides/safety/humanoids). The G1 is a legged robot that can fall, and the guidance there on supporting it during early testing prevents the most common damage.

The first-run sequence is covered end to end in the **[G1 Development Guide](/guides/unitree/g1_dev_guide)**, the most detailed guide on this site. In outline:
...
If you only need the robot on the internet rather than a development setup, the [Internet Connection Guide](/guides/unitree/g1_internet_guide) is the shorter path.
...
Use a **wired** connection for anything touching low-level control — WiFi dropouts can stall a control loop and drop the robot. WiFi is fine for high-level work and for internet access; the [Internet Connection Guide](/guides/unitree/g1_internet_guide) covers both methods, including sharing your host machine's connection.
...
The [G1 Development Guide](/guides/unitree/g1_dev_guide) has fuller diagrams of the internal wiring and of the two SDK approaches.
...
This is what [Operational Safety](/guides/operational-safety#if-something-goes-wrong) means by "hit the emergency stop" on the G1.
...
| --- | --- | --- |
| [G1 Development Guide](/guides/unitree/g1_dev_guide) | Hardware layout, network setup, SDK and software architecture, first motion example | Setting the robot up, or starting to write code against it |
| [G1 Diagnostics Guide](/guides/unitree/g1_diag_guide) | Reading diagnostic data and alarm information through the mobile app | Something is wrong and you need to know what |
| [G1 Internet Connection Guide](/guides/unitree/g1_internet_guide) | Joining the development PC to WiFi, or sharing your host's connection | The robot needs internet access, e.g. to install packages |

**[All G1 guides](/guides/tags/g1)** — generated from the `g1` tag, so anything published later appears there without this page being edited.
...
See [the full answer](/guides/operational-safety#while-you-are-developing) and the [Internet Connection Guide](/guides/unitree/g1_internet_guide).
...
- [How often do I need to lubricate the joints?](/guides/maintenance/humanoids) — and what to do about stiffness or play
