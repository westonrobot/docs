...
Connect to the **PC1 Switch Ethernet port** (labelled *Ethernet 1* below) — a single cable puts you on the same subnet as both onboard computers. This diagram covers the full internal topology, including the multimedia and external-I/O paths that most SDK work never touches:
...
flowchart TB
    ...
    subgraph MCZ["Motion Control Computing Unit"]
        direction TB
        USBH["USB3.0 hub"] --> MCCU["Motion Control<br/>Computing Unit"]
        MCCU --> AC1["Audio Chip"] --> AMP["Power Amplifier"] --> SPK["Speaker"]
        MCCU --> AC2["Audio Chip"] --> MIC["Microphone"]
        MCCU --> SW["Switch"] --> MCUX["MCU"] -->|PWM| LED["LED"]
        MCCU --> CAM["Front Camera"]
    end
    ...
    subgraph UDZ["User Development Unit"]
        UDU["User Development Unit"]
    end

    LIDF["Front LiDAR<br/>192.168.124.20"]
    LIDB["Back LiDAR (optional)<br/>192.168.124.21"]
    PASS["Passthrough chip"]
    EXT["External interfaces<br/>CAN +24V · 485 +12V · CAN +BAT · 485 +24V"]
    ...
    class MCCU,AC1,AC2,SW,MCUX,LED,AMP,SPK,MIC,CAM,USBH mc
    class UDU ud
    class SW1,SW2 net
    class LIDF,LIDB,PASS,EXT periph
```

*Redrawn from Unitree's official [About A2 — Internal Devices](https://support.unitree.com/home/en/A2_SDK_Development_Guide/about_a2) page, which hosts the original diagram.*
