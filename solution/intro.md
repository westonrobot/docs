...
Two toolboxes make that work, and they are used in that order. The **Robot Deployment Toolbox** comes first: it prepares a site once, turning a 3D scan of the building into the map robots navigate by. The **Robot Management Toolbox** is what the site is then run from, every day after that.
