...
Blocked on the first item; everything below it depends on having the files. `docs/design/file-hosting.md` carries the reference design, `infra/README.md` the runbook. Phases 1–3 are written and locally verified — the infrastructure, the publish script, the `<Downloads>` component and the CI gates all exist. **None of it has run against an account**, so every item below is an act of operation rather than of coding.
...
- [ ] **IAM Identity Center is not enabled in this account** — `aws sso-admin list-instances` returns nothing. Until it is, attach `wr-files-publish` and `wr-files-read` to an IAM group with console access and enforced MFA rather than to permission sets.
...
- [ ] Exercise `scripts/publish-files.py --publish` against the real bucket once it exists. The dry run, key derivation, refusal of unparseable names, page detection and link substitution are all verified locally; the upload, index and invalidation calls have never run.
