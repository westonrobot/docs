...
- [ ] Enforce MFA on the publishers. Identity Center is not enabled (`aws sso-admin list-instances` returns nothing), so this is an IAM condition on the group rather than a permission-set setting. Not yet applied.
