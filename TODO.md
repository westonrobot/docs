...
- [ ] **IAM Identity Center is not enabled in this account** — `aws sso-admin list-instances` returns nothing. Until it is, attach `DocsDownloadPublish` and `DocsDownloadRead` to an IAM group with console access and enforced MFA rather than to permission sets.
