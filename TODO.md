...
- [ ] **Finish the access-key rotation** started 2026-09-01. The new key is configured and verified against S3, CloudFormation, IAM and the publish path; the superseded one is deactivated but not yet deleted. Leave a few days to catch anything else configured with it, then delete — deactivated is not rotated. Key IDs are in the IAM console, not here.
