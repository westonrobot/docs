...
- [ ] Build the publish script — scan `_files/`, parse the naming convention, checksum, upload to the inbox, substitute the published URL in the page. Phase 2; needs the bucket to exist first, so not startable yet. A Claude Code skill wraps it; the script must run standalone for CI and for anyone without the skill.
