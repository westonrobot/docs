...
**The domain** is `westonrobot.net` because it is in Route 53 in this account and directly administered, while `westonrobot.com` is on Cloudflare — verified: `dig +short NS westonrobot.com` returns `brian`/`marlowe.ns.cloudflare.com`, and Route 53 in our account holds only the `.net` zone. The defect being fixed was a permanent URL living in a namespace nobody here administered, so preferring the tidier TLD over the one under direct control would repeat the shape of that mistake for the sake of appearances. It also makes the deploy fully automatic: ACM creates and resolves the validation records itself and the alias comes with the stack, where `.com` would need both added by hand in Cloudflare, at every renewal, by whoever holds that account.
...
**Terminate TLS on our own Caddy instance instead of CloudFront.** Considered 2026-08-31: the team already runs Caddy, it gets automatic certificates from Let's Encrypt, and it would remove the us-east-1 certificate step. Rejected on two grounds. **Cost runs the wrong way** — EC2 charges internet egress per GB from the first byte, while CloudFront's first 1 TB per month is free in perpetuity (verified against the AWS Pricing API, §9 of the design doc), so at any volume this store will see, the managed service is free and the self-hosted one is not. **And the lifecycle is wrong**: the instance offered was a development box, while D1 and D4 make these URLs permanent — anchoring a permanent contract to something the team treats as disposable reproduces the shape of the defect being fixed. The friction that motivated it has also gone, since the certificate, its validation records and the alias all come with the stack.

Worth recording that this remains *available* rather than closed off. Delivery is the swappable layer in the model, so moving to Caddy later would change no customer-facing URL — which is the point of D1.
