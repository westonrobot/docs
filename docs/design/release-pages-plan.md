...
**Source material.** The internal briefing at `wr_arch_docs/docs/release/202608/release-1-briefing.md`, the release page at `wr_arch_docs/src/overview/releases/r1.md`, and the implementing repositories `wr_fleet_management`, `wr_deployment_toolbox` and `wra_topometric_graph`. The briefing is internal and cannot be ported directly — see §4.
...
| **D-R4** | Pages document the model, the workflow, and what an action commits you to — not which button to press. | Step-by-step UI walkthroughs. Rejected: the Fleet Management UI is designed to be self-explanatory, so a walkthrough duplicates the screen, rots when a control moves, and is the most screenshot-heavy content to maintain. Recorded as a general rule in `ia-proposal.md` §7. |
...
| **D-R6** | The event vocabulary splits by audience: operator-facing display names and urgency live on the Fleet Management page; the raw tokens are integration reference and link out under D9. | Publishing the token list as operator reference. Rejected because operators never see tokens — `ppe_missing` renders as "Safety gear missing". `eventtypes.go` is explicit that the raw token "must not become the title of an alert". |
...
3. **Role strings in the UI** — the docs must use the exact displayed strings; `roles.go` gives the API names, and the UI labels have not been confirmed against them.
...
5. **Component version visibility** — whether a customer can see component versions at all. This decides whether the version mapping table is customer-facing or support-facing.
...
7. **D9's rationale is missing** — it cites `wr_unitree_support/docs/PUBLISHING.md`, which does not exist anywhere in the workspace. D9 is the rule that sends SDK reference off-site, so it is load-bearing for D-R6.
